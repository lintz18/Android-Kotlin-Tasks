...
import net.offshoretech.garcitech.tasks.R
import net.offshoretech.garcitech.tasks.data.SortOrder
import net.offshoretech.garcitech.tasks.data.Task
import net.offshoretech.garcitech.tasks.data.TasksAdapter
import net.offshoretech.garcitech.tasks.databinding.FragmentTasksBinding
import net.offshoretech.garcitech.util.exhaustive
import net.offshoretech.garcitech.util.onQueryTextChanged
            ...
            viewModel.taskEvent.collect { event ->
                when (event) {
                    ...
                    is TasksViewModel.TaskEvent.NavigateToAddTaskScreen -> {
                        val action = TasksFragmentDirections.actionTasksFragmentToAddEditTaskFragment(null ,"Nova tasca")
                            findNavController().navigate(action)

                        //Podríamos hacer directamente la navegación
                            //findNavController().navigate(R.id.action_tasksFragment_to_addEditTaskFragment)
                        //Pero no podemos pasar argumentos
                    }
                    is TasksViewModel.TaskEvent.NavigateToEditTaskScreen -> {
                        val action = TasksFragmentDirections.actionTasksFragmentToAddEditTaskFragment(event.task, "Editar tasca")
                        findNavController().navigate(action)
                    }
